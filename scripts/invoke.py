...
def main():
    ...
    if opt.socket:
        main_loop_socket(gen, opt, infile, HOST, PORT)
    else:
...
def main_loop_socket(gen, opt, infile,  HOST, PORT):
    """prompt/read/execute loop"""
    done = False
    path_filter = re.compile(r'[<>:"/\\|?*]')
    last_results = list()
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while not done:
            ...
            print('Outputs:')
            # send output image file path back to socket
...
def invoke_ai_web_server_loop(gen, gfpgan, codeformer, esrgan):
    ...
    # Change working directory to the stable-diffusion directory ????? WHY
    """
    os.chdir(
